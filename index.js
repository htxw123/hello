...
function decryptWeMsg(encryptStr) {
  ...
  return buf.slice(20, 20 + msgLen).toString();
}
...
app.post('/', async (req, res) => {
  try {
    ...
    chatHistory.set(uid, messages);

    res.send(genReply(msg, reply));
  } catch (err) {
    res.send(genReply({FromUserName:"",ToUserName:""}, "哥哥这边出错啦，宝宝重新发一句好不好"));
  }
});

// 封装企微回复xml
function genReply(msg, text) {
  return `<xml>
